      ...
      child: ClipRRect(
        ...
        child: Stack(fit: StackFit.expand, children: [
          park.imageUrl.startsWith('http') 
              ...
              : Image.asset(park.imageUrl, fit: BoxFit.cover,
                  errorBuilder: (_, __, ___) => Container(
                      color: AppColors.primaryDark,
                      child: const Icon(Icons.park, color: Colors.white54, size: 60))),
          Container(
...
class SightingCard extends StatelessWidget {
  ...
  String _getEmojiForAnimal(String name) {
    final lower = name.toLowerCase();
    if (lower.contains('leopard')) return '🐆';
    if (lower.contains('elephant')) return '🐘';
    if (lower.contains('bear')) return '🐻';
    if (lower.contains('peacock')) return '🦚';
    if (lower.contains('crocodile')) return '🐊';
    if (lower.contains('monkey') || lower.contains('langur')) return '🐒';
    if (lower.contains('bird') || lower.contains('fowl')) return '🦅';
    if (lower.contains('snake') || lower.contains('cobra')) return '🐍';
    return '🐾';
  }
}

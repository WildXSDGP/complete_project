...
class _LoginTabState extends State<LoginTab> {
  ...
  @override
  void dispose() {
    _inputCtrl.dispose(); _passwordCtrl.dispose(); _otpCtrl.dispose();
    super.dispose();
  }
  ...
  Future<void> _loginWithEmail() async {
    ...
    if (!mounted) return;
    if (result.isSuccess) {
      widget.showSnackBar('🎉 Welcome back!', isError: false);
      await Future.delayed(const Duration(milliseconds: 400));
      _goHome();
    } else {
    ...
    return Column(
      ...
      children: [
        ...
        if (_input.isNotEmpty && (_isPhone || _isEmail) && !_showPasswordField) ...[
          const SizedBox(height: 8),
          Row(children: [
            Icon(_isPhone ? Icons.sms_rounded : Icons.email_rounded,
